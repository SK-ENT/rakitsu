...
missed exactly that kind of false positive — a `unix`
build-tag claim that cited real code but drew a wrong conclusion from it.
Don't widen this back to a truth-check without first
...
# actually in the diff. Starting point, not a calibrated cutoff — re-check
# against real test cases before trusting this in prod CI unsupervised.
